...
const isWordBlurred = ref(true); // 当前单词是否模糊
...
watch(
  ...
  () => {
    ...
    wordList.value = Array.from(current).map((item, index) => ({
      word: item,
      input: "",
      isTrue: undefined,
    }));
...
// 下一个
const pageNext = () => {
  if (!wordList.value.every((item) => item.isTrue === true)) {
...
const onKeyDown = (index: number, event: KeyboardEvent) => {
  ...
  if (
    ...
    if (current.input && index < wordList.value.length - 1) {
      ...
      wordList.value[index + 1].isTrue =
        wordList.value[index + 1].word === event.key;
      nextTick(() => {
        const inputs = inputRefs.value as HTMLInputElement[];
        inputs[index + 1].focus();
      });
